...
export const middlewareActions = {
  [actionTypes.USER_SIGNIN]: (kwalitoSDK, action) => new Promise((resolve) => {
    assert(action.login, 'Please enter your login (email)');
    assert(action.password, 'Please enter your password');
    resolve(kwalitoSDK.userLogin({ login: action.login, password: action.password })
      .then((user) => {
        action.user = user;
      }));
  }),

  [actionTypes.USER_SIGNUP]: (kwalitoSDK, action) => new Promise((resolve) => {
    assert(action.userInfo.firstName, 'Please enter your first name');
    assert(action.userInfo.lastName, 'Please enter your last name');
    assert(action.login, 'Please enter your login (email)');
    assert(action.password, 'Please enter your password');
    resolve(kwalitoSDK.userSignupOrLogin({
      login: action.login,
  ...
  [actionTypes.USER_SIGNOUT]: (kwalitoSDK, action) => new Promise((resolve) => {
    resolve(kwalitoSDK.userLogout()
      .then((user) => {
        action.user = user;
      }));
  }),

  [actionTypes.DIET_TOGGLE_SELECT]: (kwalitoSDK, action, store) => new Promise((resolve) => {
    assert(action.id, 'The id of the diet is mandatory');
    resolve(kwalitoSDK.userGet()
      .then((user) => {
  ...
  [actionTypes.DIET_SET_OPTIONS]: (kwalitoSDK, action, store) => new Promise((resolve) => {
    assert(action.id, 'The id of the diet is mandatory');
  ...
  [actionTypes.INGREDIENT_SEARCH]: (kwalitoSDK, action, store) => new Promise((resolve) => {
    assert(action.query, 'Please write your query');
    resolve(kwalitoSDK.ingredientSearch(action.query)
      .then((result) => {
  ...
  [actionTypes.INGREDIENT_ADD]: (kwalitoSDK, action, store) => new Promise((resolve) => {
    assert(action.ingredient, 'The ingredient is mandatory');
    resolve(kwalitoSDK.ingredientAdd(action.ingredient)
  ...
  [actionTypes.INGREDIENT_REMOVE]: (kwalitoSDK, action, store) => new Promise((resolve) => {
    assert(action.ingredient, 'The ingredient is mandatory');
    resolve(kwalitoSDK.ingredientRemove(action.ingredient)
      .then((newUserIngredients) => {
        const state = store.getState();
        const user = _.assign({}, state.kwalito.user);
        user.ingredients = newUserIngredients;
